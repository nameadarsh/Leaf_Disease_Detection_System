...
def load_models():
    ...
    # Load U2Net
    u2net = U2NET(3, 1)
    u2net_path = os.path.join(current_dir, "U-2-Net/saved_models/u2net.pth")
    ...
    resnet_path = os.path.join(current_dir, "models/best_model.pth")
...
def preprocess_image(image_path, target_size=320):
    """Preprocess image for U2Net from file path"""
...
def preprocess_pil_image(pil_image, target_size=320):
    """Preprocess PIL Image for U2Net"""
...
def remove_background(image_input, u2net):
    ...
    if isinstance(image_input, str) or isinstance(image_input, Path):
        # If it's a file path, preprocess normally
        image = preprocess_image(image_input)
        original = cv2.imread(str(image_input))
        original = cv2.cvtColor(original, cv2.COLOR_BGR2RGB)
    else:
        # If it's a PIL Image, preprocess differently
        image = preprocess_pil_image(image_input)
        original = np.array(image_input.convert('RGB'))
